...
export function CoinsOptionsSection() {
  ...
  const {
    ...
    coins?.map((coin: CoingeckoCryptoCoinInfo) => coin.id),
  );
  ...
  if (errorsCoinsMarketValues)
    return "Oops, ocorreu um erro: " + errorsCoinsMarketValues.message;

  const chartData = coinsMarketValues
  ...
  return (
    <section>
      ...
      {isPendingCoinsMarketValues ? (
        <Skeleton className={cn("h-72", "w-full", "rounded-lg")} />
      ) : errorsCoinsMarketValues ? (
